(function ($) {
    ...
    //*************Product Helper Core functions********************************************//
    // Summary: this extension will handle all ultilities for Product
    Helpers.ProductSupporterCore = Helpers.extend({
        myProductColorDefault: "#5491f6",
        listColorDefaultForCompetitor:['#78c301', '#ffa400', '#009794', '#f36971', '#174781', '#ff6300', '#ce0000',
                    '#5491f6', '#946a47', '#2c5195', '#4b7717', '#78c301', '#005d5d', '#bd535b'],
        
        resetCompetitorProductsColour: function (competitorProductList) {
            var listColorDefaultForCompetitor = Helpers.productHelper.listColorDefaultForCompetitor, i = 0;
            competitorProductList.forEach(function (product) {
                product.Color = (i < listColorDefaultForCompetitor.length) ?
                        listColorDefaultForCompetitor[i] : '#000000'; i++;
            });
        },
        
        resetMyProductsColour: function (myProducts) {
            myProducts.forEach(function (product) { product.Color = Helpers.productHelper.myProductColorDefault; });
        },
        
        isProductExist: function (newProduct, myProductList, myCompetiorList) {
            var isExisted = false;

            for (var i = 0; i < myProductList.length; i++) {
                if (myProductList[i].ProductId === newProduct.ProductId) {
                    isExisted = true;
                    break;
                }
            }
            ...
            if (!isExisted) {
                for (var j = 0; j < myCompetiorList.length; j++) {
                    if (myCompetiorList[j].ProductId === newProduct.ProductId) {
                        isExisted = true;
                        break;
                    }
                }
    ...
    Helpers.productHelper = new Helpers.ProductSupporterCore();
})(jQuery);